...
static unsigned long get_cas_id();
...
unsigned long get_cas_id() {
    static unsigned long cas_id = 0;
    return ++cas_id;
}
...
/*@null@*/
// 客户端的消息组成了item链表，item链表相当于一个LRU队列，按最后访问时间排序
item *do_item_alloc(char *key, const size_t nkey, const int flags, const rel_time_t exptime, const int nbytes) {
    ...
    it->nkey = nkey;
    it->nbytes = nbytes;
    //strcpy_s(ITEM_key(it), nkey, key);
    strcpy_s(ITEM_key(it), nbytes, key);
    it->exptime = exptime;
...
char *do_item_cachedump(const unsigned int slabs_clsid, const unsigned int limit, unsigned int *bytes) {
    ...
    while (it != NULL && (limit == 0 || shown < limit)) {
        ...
        if (bufcurr + len + 6 > memlimit)  /* 6 is END\r\n\0 */
            break;
        strcpy_s(buffer + bufcurr, sizeof(temp), temp);
        bufcurr += len;
        shown++;
        it = it->next;
    }
...
char* do_item_stats_sizes(int *bytes) {
    ...
    *bytes = 0;
    for (i = 0; i < num_buckets; i++) {
        if (histogram[i] != 0) {
            *bytes += sprintf_s(&buf[*bytes], "%d %u\r\n", i * 32, histogram[i]);
        }
    }
    *bytes += sprintf_s(&buf[*bytes], 2 * 1024 * 1024,"END\r\n");
    free(histogram);
    return buf;
}
