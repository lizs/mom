...
#define CONFIG_MAX_BYTES 64 * 1024 * 1024
#define CONFIG_FACTOR 1.25
...
static void complete_nread(conn *c) {
	...
	//printf("item:%s, item_delta:%s\n", (char*) &((it)->end[0]), (char*) &((it)->end[0]) + (it)->nkey + 1);
	printf("str:%s\n", ITEM_data(it));
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    if (settings.managed) {
        ...
        if (bucket == -1) {
			...
			printf("CLIENT_ERROR no BG data in managed mode\n");
            return;
        }
        c->bucket = -1;
    ...
    do {
        while(key_token->length != 0) {
            ...
            if(nkey > KEY_MAX_LENGTH) {
                ...
                STATS_UNLOCK();
                //out_string(c, "CLIENT_ERROR bad command line format");
				printf("CLIENT_ERROR bad command line format");
                return;
            }
            ...
            if (it) {
                ...
                if(return_cas == true)
                {
                  ...
                  if (suffix == NULL) {
					...
					printf("SERVER_ERROR out of memory making CAS suffix");
                    item_remove(it);
                    return;
                  }
				...
				printf(">%d sending key %s values:%s\n", c->sfd, ITEM_key(it), ITEM_data(it));
    ...
    */
        ...
        //|| (c->udp && build_udp_headers(c) != 0)) 
	{
		...
		printf("SERVER_ERROR out of memory writing get response");
    }
    else {
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    c->rlbytes = it->nbytes; // nbytes = data_len+2
	//printf("process_update_command ritem:%s, rlbytes:%d\n", c->ritem, c->rlbytes);
    c->item_comm = comm;
	// 这里设置成nread，表示数据已经全部准备好了，可以进行下面的操作了
	// 3/8 数据并不是全准备好了，只是准备好了空间，但是具体的数据还需要通过read来从buffer里面读取
 ...
 */
char *do_add_delta(conn *c, item *it, const bool incr, const unsigned  delta, char *buf) {
    char *ptr;
    unsigned long value;
    int res;
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens >= 3 &&
        ...
        if (sscanf_s(tokens[1].value, "%u:%u", &bucket,&gen) == 2) {
        ...
        if (sscanf_s(tokens[1].value, "%u", &bucket) == 1) {
            if ((bucket < 0) || (bucket >= MAX_BUCKETS)) {
                out_string(c, "CLIENT_ERROR bucket number out of range");
                return;
            }
...
static int try_read_command(conn *c) {
    ...
    *el = '\0';
	...
	printf("ready to enter process_command\n");
...
// 事件逻辑在这里，先测试一下，这个没有问题就可以开始集成libuv了
// listen主要用来构建struct conn
// r/w主要是用来从slab中取出数据生成item
// nr/nw主要就是通过cmd来对item指向的slab中的数据进行具体的处理
static void drive_machine(conn *c) {
    bool stop = false;
    int sfd, flags = 1;
    socklen_t addrlen;
    ...
    while (!stop) {
        ...
        case conn_read:
			// 这里生成item，返回的时候item已经准备好了，并且状态已经置位成了nread
...
static int try_read_network_test(conn *c, char data[]) {
	int gotdata = 0;
	int res;
	int avail;
	...
	while (1) {
		...
		* mode.  */
		...
		//res = read(c->sfd, c->rbuf + c->rbytes, avail);
		res = c->rlbytes;
		strcpy_s(c->rbuf, strlen(data)+1, data);
		if (res > 0) {
			gotdata = 1;
			c->rbytes += res;
			if (res == avail) {
	...
	printf("rbuf:%s, rbytes:%d\n", c->rbuf, c->rbytes);

	return gotdata;
}


void nread_test(conn *c)
{
	/*  now try reading from the socket */
	//int res = read(c->sfd, c->ritem, c->rlbytes);
	int res = c->rlbytes;
	strcpy_s(c->ritem, c->rlbytes+1, c->rbuf);
	if (res > 0) {
		c->ritem += res;
		c->rlbytes -= res;
	}
...
int main (int argc, char **argv) {
	...
	// 客户端的消息到memcache会被拼接成item，然后给对应的item来分配mem
    item_init();
    //stats_init();
	// hash init 用于数据的存储架构
    assoc_init();
	...
	/* init subclass and preallocate if it's true */
	// slabclass->slab->chunk->item(key-value)
	slabs_init(settings.maxbytes, settings.factor, preallocate);

	conn *c_one = conn_new_test();
	if (NULL == c_one)
	{
		printf("create conn c_one error\n");
	}
	else
	{
		printf("create conn ok, c_one conn state:%d\n", c_one->state);
	}
#if 0
	if (try_read_command(c) != 0) {
		printf("get slab ok\n");
	...
	{
		printf("get slab error\n");
	}
#endif
	//构建参数
	token_t tokens[MAX_TOKENS];
	size_t ntokens;
	//char *command = "get myboy";
#if 1
	char set_command[] = "add myboy 0 0 5\r\n";
	char data[] = "12345\r\n";
	int comm = NREAD_ADD;
	ntokens = tokenize_command(set_command, tokens, MAX_TOKENS);
	...
	// set method
	process_update_command(c_one, tokens, ntokens, comm, false);
	int rec_result = try_read_network_test(c_one, data);
	if (rec_result)
	{
		printf("moni recv OK!\n");
	}
	...
	{
		printf("moni recv ERROR!\n");
	}

	printf("rbytes:%d, rlbytes:%d, ritem:%s\n", c_one->rbytes, c_one->rlbytes, c_one->ritem);
	nread_test(c_one);
	//complete_nread(c_one);
#endif
	//sleep(5);


	conn *c_two = conn_new_test();
	if (NULL == c_two)
	{
		printf("create conn c_two error\n");
	}
	else
	{
		printf("create conn ok, c_two conn state:%d\n", c_two->state);
	}

	char get_command[] = "get myboy";
	ntokens = tokenize_command(get_command, tokens, MAX_TOKENS);
	//for (int j= 0;j < ntokens;j++)
	//{
	//	printf("cmd:%s, len:%d\n", tokens[j].value, tokens[j].length);
	//}
	// get method
	process_get_command(c_two, tokens, ntokens, false);
	//nread_test(c_two);
	//complete_nread(c_two);
#if 0
	char data1[] = "";
	rec_result = try_read_network_test(c_two, data1);
	if (rec_result)
	{
		printf("moni recv OK!\n");
	}
