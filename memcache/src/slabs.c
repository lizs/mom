...
typedef struct {
	...
	// 回收的item链表，item回收时不是直接收回item的内存，而是先挂到slots尾部，之后可以拿来复用
    void **slots;           /* list of item ptrs */ 
    unsigned int sl_total;  /* size of previous array */
    unsigned int sl_curr;   /* first free slot */

	// end_page_ptr指向最后一个 slab 中的空闲内存块
    void *end_page_ptr;         /* pointer to next free item at end of page, or 0 */
	// end_page_free 表示最后一个 slab 中还剩下多少个空闲 chunk
    unsigned int end_page_free; /* number of items remaining at end of last alloced page */

    unsigned int slabs;     /* how many slabs were allocated for this class */

	// 一个slabclass中有多个slab，slab就保存在slab_list上
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    while (++i < POWER_LARGEST && size <= POWER_BLOCK / 2) {
		...
		/* POWER_BLOCK : one slabclass's size = size*perslab  DEFAULT : 1M */
		/* chunk number per slabclass */
        slabclass[i].perslab = POWER_BLOCK / slabclass[i].size;
		/* chunk size increase factor */
	...
	printf("slabclass numbers:%d\n", power_largest);

    /* for the test suite:  faking of how much we've already malloc'd */
    {
        //char *t_initial_malloc = getenv("T_MEMD_INITIAL_MALLOC");
		char *t_initial_malloc;
		size_t t_initial_len;
		errno_t err = _dupenv_s(&t_initial_malloc, &t_initial_len, "T_MEMD_INITIAL_MALLOC");
		if (err)
		{
			exit(-1);
		}
		...
		//printf("str:%s, len:%d\n", t_initial_malloc, t_initial_len);
    }
...
/*@null@*/
// 根据id来取一个slab
// 分配原则：如果slots中有空闲的就取空闲的，没有就分配新的
...
// 归还items，把ptr指向的item挂在slots空闲链表的最前面
    ...
    *buflen = 0;
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            //bufcurr += sprintf_s(bufcurr, "STAT %d:chunk_size %u\r\n", i, p->size);
            //bufcurr += sprintf_s(bufcurr, "STAT %d:chunks_per_page %u\r\n", i, perslab);
            //bufcurr += sprintf_s(bufcurr, "STAT %d:total_pages %u\r\n", i, slabs);
            //bufcurr += sprintf_s(bufcurr, "STAT %d:total_chunks %u\r\n", i, slabs*perslab);
            //bufcurr += sprintf_s(bufcurr, "STAT %d:used_chunks %u\r\n", i, slabs*perslab - p->sl_curr);
            //bufcurr += sprintf_s(bufcurr, "STAT %d:free_chunks %u\r\n", i, p->sl_curr);
            //bufcurr += sprintf_s(bufcurr, "STAT %d:free_chunks_end %u\r\n", i, p->end_page_free);
            total++;
        }
    }
    //bufcurr += sprintf_s(bufcurr, "STAT active_slabs %d\r\nSTAT total_malloced %llu\r\n", total, (unsigned long long)mem_malloced);
    //bufcurr += sprintf_s(bufcurr, "END\r\n");
    *buflen = bufcurr - buf;
    return buf;
}
